...
class arg:
    def __init__(self):
        ...
        self.num_classes = 286

args = arg()
model = ImagingModel(args)
sd = torch.load('logs/resnet50/models/best_model_1697163613_783395_epoch187.t7')['state_dict']
for k in list(sd.keys()):
    if 'fc' in k:
        sd[k.replace('model.fc', 'classifier')] = sd[k]
        del sd[k]
    else:
        sd[k.replace('model', 'encoder')] = sd[k]
        del sd[k]

model.load_state_dict(sd)

print(model)
# print(ImagingModel(args))