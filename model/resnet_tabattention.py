# adapted from https://github.com/SanoScience/Tab-Attention/blob/main/src/models/resnet_tabattention.py
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, conv_builder, stride=1, downsample=None, n_frames_last_layer=2,
        ...
        midplanes = (inplanes * planes * 3 * 3) // (inplanes * 3 + 3 * planes)
...
class VideoResNet(nn.Module):
    ...
    def __init__(self, block, conv_makers, layers, stem, num_classes=1, zero_init_residual=False, head=False,
                 n_frames_last_layer=1, input_last_layer=(7, 7), n_tab=6, ):
    ...
    def forward(self, image, tab_line=None, *args, **kwargs):
        x = image
        tab = tab_line.unsqueeze(1)
        x = self.stem(x)
...
def ResNetTabAttention(input_size=(224, 224), n_frames=16, n_tab=6, **kwargs):
    ...
    return _video_resnet(block=BasicBlock, conv_makers=[Conv2DSimple] * 4, layers=[2, 2, 2, 2], stem=BasicStem,
