...
class DAFT(nn.Module):
    def __init__(self, tab_emb_dim, num_classes, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        resnet = resnet50(pretrained=True, num_classes=num_classes)
    ...
    def forward(self, tab_line, image, *args, **kwargs):
        ...
        feat = self.pool(feat_after_daft).flatten(-2).squeeze(-1)
...
class PPNet(nn.Module):
    def __init__(self, tab_emb_dim, num_classes, *args, **kwargs) -> None:
        ...
        self.fc1 = nn.Linear(tab_emb_dim, resnet.conv1.out_channels)
        self.fc2 = nn.Linear(resnet.conv1.out_channels, resnet.layer1[-1].conv3.out_channels)
    ...
    def forward(self, tab_line, image, *args, **kwargs):
        ...
        feat = self.pool(feat).flatten(-2).squeeze(-1)
        logit = self.classifier(feat)
