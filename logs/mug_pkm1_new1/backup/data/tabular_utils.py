...
class TextEmbedder(DefaultEmbedder):
    ...
    def get_line(self, df: pd.DataFrame, meta_info: dict, index: int) -> np.ndarray:
        ...
        if self.model == 'clip':
          ...
          # roberta or medclip
          line_embd = self.tokenizer(line_sentence, return_tensors='pt', padding=True) if self.model=='medclip' else self.tokenizer(line_sentence, return_tensors='pt', padding=True)
