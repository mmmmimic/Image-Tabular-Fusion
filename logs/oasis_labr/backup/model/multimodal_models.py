...
import torch
from .modules import DinoFusion
...
class LogitEmbeddingLayer(nn.Module):
    def __init__(self, feat_dim = 1024, zero_conv = True, dropout=0.5, *args, **kwargs) -> None:
...
class MultimodalModel(nn.Module):
    def __init__(self, tab_emb_dim, num_classes, feat_dim = 1024, fusion='cat', 
                 ...
                 lora_r=4, phi=0.01, reduction=8, dropout=0.5, ratio=1, latent_dim=64, prefix_num=4, 
                 *args, **kwargs) -> None: 
        super().__init__(*args, **kwargs)
        '''
        Structure:
    ...
    def _init_fuser(self):
        if self.fusion == 'add':
            ...
            self.fused_dim = self.latent_dim*4
            self.dino_fusion = DinoFusion(self.feat_dim, self.phi, self.reduction, self.dropout, self.ratio, self.latent_dim)
    ...
    def _build_tabular_encoder(self, encoder_name):
        if encoder_name == 'mlp':
            ...
            model = clip_bert(self.prefix_num)
            assert self.feat_dim == 512,f'embedding dimension {self.feat_dim} is illegal'
            if self.frozen_tab:
...
class ResMultimodalModel(MultimodalModel):
    def __init__(self, tab_emb_dim, num_classes, feat_dim = 1024, fusion='cat', 
        ...
        super().__init__(tab_emb_dim, num_classes, feat_dim, fusion, 
                 image_encoder, tab_encoder, frozen_tab, lora_r, phi, reduction, dropout, ratio, latent_dim, prefix_num,*args, **kwargs)
        self.embedder = LogitEmbeddingLayer(feat_dim=feat_dim, zero_conv=zero_conv, dropout=dropout)
