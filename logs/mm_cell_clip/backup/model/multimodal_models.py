...
class MultimodalModel(nn.Module):
    def __init__(self, tab_emb_dim, num_classes, emb_dim = 1024, fusion='cat', image_encoder='rn50', tab_encoder='mlp', *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        '''
        Structure:
            image -> visual_encoder -> image features   |
                                                        | -> fusion -> classifier
    ...
    def _build_image_encoder(self, encoder_name):
        if encoder_name == 'rn50':
            self.image_encoder = resnet50(pretrained=False, num_classes=self.emb_dim)
        elif encoder_name == 'rn50_clip':
            self.image_encoder, _ = clip.load('RN50', device='cpu')
            assert self.emb_dim == 1024,f'embedding dimension {self.emb_dim} is illegal'
        elif encoder_name == 'rn_50_clip_zero':
            self.image_encoder, _ = None
        else:
            raise ValueError        
