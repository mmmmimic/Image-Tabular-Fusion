...
class ClipTransformer(nn.Module):
    def __init__(self, prefix_num, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        model, _ = clip.load("ViT-B/16", device='cpu')
        self.transformer = model
        del self.transformer.visual
        embedding_dim = 512
        assert prefix_num%2==0
        self.prefix_embedding = nn.Parameter(torch.randn(1, prefix_num, embedding_dim))
        torch.nn.init.kaiming_normal_(self.prefix_embedding)
    ...
    def add_prefix_to_input(prefix_embedding, text_embeddings):
        ...
        # 拼接 prefix 和原始文本嵌入
        return torch.cat([text_embeddings[:,[0],:], prefix, text_embeddings[:,1:-prefix.size(1)-1,:], text_embeddings[:,[-1],:]], dim=1)
...
def clip_bert(prefix_num):
    return ClipTransformer(prefix_num)
