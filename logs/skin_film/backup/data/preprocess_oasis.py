...
##### CONSTANTS ######
ROOT_PATH = 'data/oasis'
TABLE_PATH = join(ROOT_PATH, 'oasis_cross-sectional-5708aa0a98d82080.xlsx')
CATEGORICAL_IDS = ['M/F', 'Educ', 'SES']
CONTINOUS_IDS = ['Age', 'MMSE', 'eTIV','nWBV','ASF', 'Delay']
LABEL = 'CDR' # depend on the folder name
######################

# read tables and convert excel to csv
def read_csv(TABLE_PATH):
    df = pd.read_excel(TABLE_PATH)
    imputer = IterativeImputer(estimator=BayesianRidge(), max_iter=10, random_state=2025)
    df[CONTINOUS_IDS] = imputer.fit_transform(df[CONTINOUS_IDS])
    # for col in CONTINOUS_IDS:
...
def collect_images(suffix='jpg'):
    imgs_dirs = glob(join(ROOT_PATH, 'Data', '*', f'*.{suffix}'))
...
def wrapup(table_df, jpg_dirs, subject_id, label):
    value_dict = {}
    ...
    for i in tqdm(range(len(subject_id))):
        for c in CATEGORICAL_IDS + CONTINOUS_IDS:
            value_dict[c].append(table_df[table_df['ID']==subject_id[i]][c].values[0])
        
    value_dict['Subject_ID'] = subject_id
    value_dict['label'] = label
    value_dict['Image_ID'] = list(map(lambda x:PurePath(x).parts[-1], jpg_dirs))
    value_dict['Image_DIR'] = jpg_dirs
...
def postprocess():
    ...
    # for label == 0
    subject_id = np.array(list(set(df['Subject_ID'].values))) 
...
if __name__ == "__main__":
    ...
    # print(table_df)
    # table_df = pd.read_csv(TABLE_PATH.replace('xlsx', 'csv'))
    # jpg_dirs = collect_images(suffix='jpg')
    # patient_names = list(map(lambda x: '_'.join(PurePath(x).parts[-1].split('_')[:3]), jpg_dirs))
    # label = list(map(lambda x: PurePath(x).parts[-2], jpg_dirs))
    # label = list(map(lambda x: {'Mild Dementia':0, 'Moderate Dementia':1, 'Non Demented':2, 'Very mild Dementia':3}[x], label))
    # print(label)
    # print(len(jpg_dirs))
    # analyse_columns(table_df)
    # processed_table_df = process_columns(table_df)
    # print(processed_table_df)
    # wrapup(processed_table_df, jpg_dirs, patient_names, label)
    postprocess()
    # # sample()
    # check_image()
