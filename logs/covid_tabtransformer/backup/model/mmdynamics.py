...
class MMDynamic(nn.Module):
    ...
    def forward(self, data_list, label=None):
        infer  = not self.MMClasifier.training
...
if __name__ == "__main__":
    model = MMDynamic([1024, 1024], [1024], 286, 0.2)
    print(model)
    tensor = [torch.rand(3, 1024), torch.rand(3, 1024)]
    label = torch.rand(3).long()
    loss, logit = model(tensor, label)
    print(loss)
    print(logit.shape)
